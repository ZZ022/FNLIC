...
class Upsampling(QuantizableModule):
    def __init__(self, upsampling_kernel_size: int):
        ...
        upsampling_kernel=F.pad(th.tensor(kernel_bicubic_alignfalse), kernel_pad) # padded with zeros by default

        # compatible with transpose conv
        upsampling_kernel=th.unsqueeze(upsampling_kernel,0)
        upsampling_kernel=th.unsqueeze(upsampling_kernel,0)
    ...
    def forward(self, decoder_side_latent: List[Tensor]) -> Tensor:
        ...
        for b in range(decoder_side_latent[0].shape[0]):
            upsampled_latent = rearrange(decoder_side_latent[-1][b:b+1,...], '1 c h w -> c 1 h w')
            ...
            for i in range(len(decoder_side_latent) - 1, 0, -1):
                ...
                target_tensor = rearrange(decoder_side_latent[i - 1][b:b+1,...], '1 c h w -> c 1 h w')
                ...
                upsampled_latent = th.cat((target_tensor, upsampled_latent), dim=0)

            if b==0:
                ret = rearrange(upsampled_latent, 'c 1 h w -> 1 c h w')
            else:
                ret = th.cat((ret, rearrange(upsampled_latent, 'c 1 h w -> 1 c h w')), dim=0)
        return ret
    ...
    def display_filters(self):
            ...
            weight=th.squeeze(weight,dim=0)
            weight=th.squeeze(weight,dim=0)
