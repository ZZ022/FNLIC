...
def softround(x: Tensor, t: float) -> Tensor:
    ...
    """
    delta = x - th.floor(x) - 0.5
    return th.floor(x) + 0.5 * th.tanh(delta / t) / math.tanh(1 / (2 * t)) + 0.5
...
class NoiseQuantizer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        noise = self.generate_kumaraswamy_noise(th.rand_like(x), self.kumaraswamy_param)
...
class STEQuantizer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Returns:
            Tensor: Quantized tensor.
        """
        # Calculate soft rounded version which will contribute to the gradient during backprop
        y_soft = softround(x, self.soft_round_temperature)

        # Calculate rounded version without gradient tracking
        with th.no_grad():
            y_round = th.round(x)

        # Combine the effects: return the rounded value but keep the gradient of the soft version
        y = y_soft + (y_round - y_soft).detach()
