...
class QuantizableModule(nn.Module):
    ...
    def measure_laplace_rate(self) -> DescriptorNN:
        ...
        for parameter_name, parameter_value in param.items():
            ...
            if self._is_weight(parameter_name):
                ...
                logging.info('Parameter name should end with ".w", ".weight", ".b" or ".bias"')
                logging.info(f'Found: {parameter_name}')
        ...
        for k, v in sent_param.items():
            ...
            rate_param[k] = -th.log2(proba).sum().item()
    ...
    def quantize(self, q_step: DescriptorNN) -> bool:
        ...
        for k, v in fp_param.items():
            ...
            sent_param = th.round(v / current_q_step)

            if sent_param.abs().max() > MAX_AC_MAX_VAL:
                # logging.info(sent_param.abs().max(), MAX_AC_MAX_VAL)
                # logging.info(f'Sent param {k} exceed MAX_AC_MAX_VAL! Q step {current_q_step} too small.')           
                return False
...
class QuantizableConv2d3x3(QuantizableModule):
    def __init__(self, possible_q_steps: Tensor, in_ft:int, out_ft:int, act_type:str='relu'):
        super().__init__(possible_q_steps)
        self.conv = nn.Conv2d(in_ft, out_ft, 3, 1, 1)
        self.act_type = act_type
        if self.act_type == 'prelu':
            self.act = nn.PReLU(num_parameters=out_ft)
    ...
    def set_params(self, weight:Tensor, bias:Tensor)->None:
