...
@dataclass
class WarmupPhase():
    """Describe one phase of a FrameEncoder warmup()
    """
...
class Preset:
    ...
    all_warmups: List[WarmupPhase] = field(default_factory=lambda : [])
    ...
    def pretty_string(self) -> str:
        ...
        s += '\nIntra Warm-up\n'
        s += '-------------\n'
        s += WarmupPhase.pretty_string_column_name() + '\n'
...
class PresetDebug(Preset):
    ...
    def __init__(self, start_lr: float = 1e-2, n_itr_per_phase: int = 100000):
        ...
        self.all_warmups: List[WarmupPhase] = [
            WarmupPhase(candidates=2, iterations=20, lr=start_lr)
        ]
